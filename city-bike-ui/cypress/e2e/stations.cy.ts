describe('Stations flow', () => {
  beforeEach(() => {
    cy.intercept('GET', '**/api/stations*', { fixture: 'stationsPage1.json' }).as('getStations')
  })
  ...
  it('lists stations and opens a station detail', () => {
    ...
    cy.intercept('GET', '**/station/*', { fixture: 'stationDetails.json' }).as('getStation')
    cy.contains('h2', 'Station informations').should('be.visible')
    cy.contains('Address').should('be.visible')
    cy.contains('Station ID').should('be.visible')
    cy.contains('Total number of journeys starting from the station').should('be.visible')
    cy.contains('Total number of journeys ending at the station').should('be.visible')
    cy.contains('The average distance of a journey starting from the station (km)').should('be.visible')
    cy.contains('The average distance of a journey ending at the station (km)').should('be.visible')
    cy.contains('Top 5 most popular departure stations for journeys ending at the station').should('be.visible')
    cy.contains('Top 5 most popular return stations for journeys starting from the station').should('be.visible')
  })
