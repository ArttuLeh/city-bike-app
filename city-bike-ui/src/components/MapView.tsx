...
import { Box, CircularProgress, Typography, List, ListItem, ListItemText, ListItemButton } from '@mui/material'
...
const MapView = ({ station, popularDepartureStations, popularReturnStations }: Props) => {
  ...
  return (
    <div>
      ...
      {isSingleStation && connectedStations.length > 0 && (
        <Box display="flex" gap={3} my={1}>
          <Typography variant="body2">
            <span style={{ color: '#1976d2', fontWeight: 'bold' }}>——</span> Departure stations for journeys ending at the {station.name}
          </Typography>
          <Typography variant="body2">
            <span style={{ color: '#d32f2f', fontWeight: 'bold' }}>——</span> Return stations for journeys starting from {station.name}
          </Typography>
        </Box>
      )}
      ...
      <Box display="flex" gap={2} flexDirection={isSingleStation ? 'row' : 'column'}>
        <Box flex={1} minWidth={0}>
          ...
          >
            ...
            {isSingleStation && stationPosition && connectedStations.map((cs, i) => (
              <Marker key={`connected-${i}`} position={cs.position}>
                <Popup>
                  ...
                  <br />
                  {cs.type === 'departure' ? 'Departure station' : 'Return station'}
                </Popup>
              </Marker>
            ))}
            ...
            {isSingleStation && stationPosition && connectedStations.map((cs, i) => {
              ...
              return (
                <Polyline
                  ...
                  pathOptions={{
                    color: isSelected ? '#ec1515' : (cs.type === 'departure' ? '#1976d2' : '#d32f2f'),
        ...
        {isSingleStation && connectedStations.length > 0 && (
          <Box sx={{ width: 280, flexShrink: 0, overflowY: 'auto', maxHeight: '70vh' }}>
            {popularDepartureStations && popularDepartureStations.length > 0 && (
              <Box mb={2}>
                ...
                <List dense>
                  {popularDepartureStations.map((station, i) => {
                    ...
                    const csIndex = connectedStations.findIndex(
                      (cs) => cs.type === 'departure' && cs.name === station.departureStationName
                    )
                    ...
                    return (
                      <ListItem key={`dep-${i}`} disablePadding>
                        <ListItemButton
                          selected={isSelected}
                          onClick={() => setSelectedIndex(isSelected ? null : csIndex)}
                          sx={{
              ...
              <Box>
                ...
                <List dense>
                  {popularReturnStations.map((station, i) => {
                    ...
                    const csIndex = connectedStations.findIndex(
                      (cs) => cs.type === 'return' && cs.name === station.returnStationName
                    )
                    const isSelected = selectedIndex === csIndex
                    return (
                      <ListItem key={`ret-${i}`} disablePadding>
                        <ListItemButton
                          ...
                          onClick={() => setSelectedIndex(isSelected ? null : csIndex)}
                          sx={{
                            borderLeft: isSelected ? '4px solid #ec1515' : '4px solid transparent',
                            py: 0.5,
                          }}
                        >
                          <ListItemText
